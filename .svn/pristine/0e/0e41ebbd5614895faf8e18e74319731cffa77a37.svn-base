...
class RoleNameKeyword extends Oauth
{
    ...
    public function keyword_list(){
        ...
        foreach($product_list as $k=>$v){
            ...
            if(!empty($v['code'])){
                echo $keyword_key['rolename_block_keyword_key'].'_'.$v['code'].'::';
                var_duMP( $redis->SMEMBERS($keyword_key['rolename_block_keyword_key'].'_'.$v['code'] ));
    ...
    public function modiftKeywordList()
    {
        ...
        RoleNameKeywordServer::modiftKeywordList();
    ...
    public function index()
    {
        ...
        $keywords = RoleNameKeywordServer::getList($data);
        $total = RoleNameKeywordServer::getCount($data);
    ...
    public function del()
    {
        ...
        if ($ids) {
            $infos = RoleNameKeywordServer::getMore($ids);

            $res = RoleNameKeywordServer::delete($infos);
            if ($res) {
                $this->rs['code'] = 0;
                $this->rs['msg'] = '操作成功';
            }
...
//        $post_data['resemble_status'] = $this->request->post('resemble_status/d', 0);
        ...
        $res = RoleNameKeywordServer::add($post_data);
...
//        $post_data['resemble_status'] = $this->request->post('resemble_status/d', 0);
        ...
        $post_data['check_type'] = $this->request->post('check_type/d', 1);
        $post_data['admin_user'] = $this->user_data['username'];
        $post_data['page'] = $this->request->request('page',  1);
        ...
        $res = RoleNameKeywordServer::edit($post_data);
    ...
    public function getEditInfo()
    {
        ...
        $res = RoleNameKeywordServer::getOne($id);
