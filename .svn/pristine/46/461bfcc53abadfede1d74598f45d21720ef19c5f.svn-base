...
class GameServerinfoStatisticsServer extends BasicServer
{

    const INSERT_NUMBER = 2000;

    public $time,$num,$platform,$start_time,$end_time,$server_info_statistics_model,$game_server_info_statistics_model;
    ...
    public function insertIntoServerInfo()
    {
        ...
        $this->game_server_info_statistics_model = new GameServerStatisticSqlServer();
        ...
        foreach($userData2 as $k2=>$v2){
            ...
            foreach($v2 as $k3=>$v3){
                ...
                @$new_arr[$k2.'_'.$v3['game_id'].'_'.$v3['server_id']]['pay_money'] += $v3['pay_money'];
                @$new_arr[$k2.'_'.$v3['game_id'].'_'.$v3['server_id']]['reg_num'] = 0;
                @$new_arr[$k2.'_'.$v3['game_id'].'_'.$v3['server_id']]['role_reg_num'] = 0;
                @$new_arr[$k2.'_'.$v3['game_id'].'_'.$v3['server_id']]['date'] =  $v3['date'];
                @$new_arr[$k2.'_'.$v3['game_id'].'_'.$v3['server_id']]['game_id'] = $v3['game_id'];
                @$new_arr[$k2.'_'.$v3['game_id'].'_'.$v3['server_id']]['add_time'] = $time;
                @$new_arr[$k2.'_'.$v3['game_id'].'_'.$v3['server_id']]['center_product_id'] =  $v3['center_product_id'];
                @$new_arr[$k2.'_'.$v3['game_id'].'_'.$v3['server_id']]['platform_id'] = $v3['platform_id'];
                @$new_arr[$k2.'_'.$v3['game_id'].'_'.$v3['server_id']]['product_id'] = $v3['product_id'];
                @$new_arr[$k2.'_'.$v3['game_id'].'_'.$v3['server_id']]['product_name'] = $v3['product_name'];

                @$new_arr[$k2.'_'.$v3['game_id'].'_'.$v3['server_id']]['server_id'] = $v3['server_id'];
                @$new_arr[$k2.'_'.$v3['game_id'].'_'.$v3['server_id']]['server_name'] = $v3['server_name'];
        ...
        foreach($userData1 as $k=>$v){
            foreach($v as $k1=>$v1){
                ...
                if(empty($v1['reg_gid']) || empty($v1['server_id'])){
                    continue;
                }

                if(isset($new_arr[$k.'_'.$v1['reg_gid'].'_'.$v1['server_id']])){

                    $new_arr[$k.'_'.$v1['reg_gid'].'_'.$v1['server_id']]['reg_num'] += $v1['reg_num'];
                    $new_arr[$k.'_'.$v1['reg_gid'].'_'.$v1['server_id']]['role_reg_num'] += $v1['role_reg_num'];
                    $new_arr[$k.'_'.$v1['reg_gid'].'_'.$v1['server_id']]['platform'] = $k;
        ...
        for ($i=0;count($new_arr) >$i*self::INSERT_NUMBER;$i++)
        {
            ...
            $res = $this->game_server_info_statistics_model->insertInfo($insertData);
            if($res){
                Db::commit();
            }else{
                Db::rollback();
