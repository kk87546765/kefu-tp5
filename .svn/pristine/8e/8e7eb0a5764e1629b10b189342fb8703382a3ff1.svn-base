...
use common\server\RoleNameBlock\RoleNameBlockWaringServer;


class RoleNameBlockWaring extends Oauth
{
    public $query = '';
    public $filter = '';
    public $es = '';
...
//        $platform_key = $this->session->get('platform_key');
        $res = RoleNameBlockWaringServer::getList($data);
        $count = RoleNameBlockWaringServer::getCount($data);
...
//    }
    ...
    public function update()
    {
        ...
        $res = RoleNameBlockWaringServer::updateStatus($data['ids']);
        $this->rs['code'] = 1;
        $this->rs['msg'] = '成功';
        $this->rs['data'] = $res;
        return return_json($this->rs);
    ...
    {
        $data['ids'] = $this->request->post('a_ids/a',[]);
        $res = RoleNameBlockWaringServer::updateStatus($data['ids'],2);
        $this->rs['code'] = $res;
        return return_json($this->rs);
    }
