...
use common\libraries\{Common,Adb};
use common\libraries\Ipip\IP4datx;


use common\libraries\ElasticSearch;
use common\sql_server\BlockSqlServer;
use common\sql_server\AdbSqlServer;



class ElasticSearchChatTestServer extends BasicServer
{
...
//                $bool['filter']['bool']['must'][]['range']['count_money']['lte'] = $money_max;
    ...
    public static  function getCheckChat($data)
    {
        ...
        if(!empty($data['uid'])){
            $bool['bool']['filter']['bool']['should'][]['term']['uid'] = $data['uid'];
        }
...
//            $v['gkey'] = $this->gamelist[$v['gkey']]['name'];
    ...
    public static function getChatByAdb($data)
    {
        $model = new AdbSqlServer();
        $where = '1';
        if(!empty($data['gkey'])){
           $where .= " and gkey = '{$data['gkey']}'";
        }
        ...
        if(!empty($data['s_date'])){
            $where .= " and time >= {$data['s_date']}";

        }

        if(!empty($data['e_date'])){
            $where .= " and time <= {$data['e_date']}";

        }

        $res = $model->getChat(['*'],$where);
        foreach ($res as &$v) {
            $v['date'] = date('Y-m-d H:i:s', $v['time']);
            $v['typename'] = self::$types[$v['type']];
//            $v['gkey'] = $this->gamelist[$v['gkey']]['name'];
